...
class CGAN(object):
    ...
    def _build_net(self):
        ...
        self.y_f = tf.placeholder(tf.float32, shape=[None, *self.image_size, 1], name='vessel_f')
        ...
        self.d_loss_real_f = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
            logits=d_logit_f, labels=tf.zeros_like(d_real_f)))
        self.d_loss = self.d_loss_real + self.d_loss_fake + self.d_loss_real_f
    ...
    def train_dis(self, x_data, y_data, y_f_data):
        feed_dict = {self.X: x_data, self.Y: y_data, self.y_f: y_f_data}
        # run discriminator
